...
async function get_rooms(email) {
  const res = await frappe.call({
    type: 'GET',
    method: 'whatsapp_integration.api.contacts.get',
    args: {
      email: email,
    },
  });
...
async function get_messages(room, user_no) {
  const res = await frappe.call({
    method: 'whatsapp_integration.api.message.get_all',
    args: {
      room: room,
      user_no: user_no,
    },
...
async function send_message(content, user, room, user_no, attachment) {
  try {
    await frappe.call({
      method: 'whatsapp_integration.api.message.send',
      args: {
        content: content,
        user: user,
        room: room,
...
async function get_settings(token) {
  const res = await frappe.call({
    type: 'GET',
    method: 'whatsapp_integration.api.config.settings',
    args: {
      token: token,
    },
  });
...
async function mark_message_read(room) {
  try {
    await frappe.call({
      method: 'whatsapp_integration.api.message.mark_as_read',
      args: {
        room: room,
      },
    });
...
async function set_typing(room, user, is_typing, is_guest) {
  try {
    await frappe.call({
      method: 'whatsapp_integration.api.message.set_typing',
      args: {
        room: room,
        user: user,
        is_typing: is_typing,
...
async function create_private_room(contact_name, mobile_no, email) {
  await frappe.call({
    method: 'whatsapp_integration.api.contacts.create',
    args: {
